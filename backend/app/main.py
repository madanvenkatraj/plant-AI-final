...
async def load_model():
    ...
    try:
        if os.path.exists(MODEL_PATH) and os.path.exists(CLASS_INDICES_PATH):
            ...
            # Fix for 'Unrecognized keyword arguments passed to BatchNormalization: {'renorm': False...}'
            # This happens when loading a model saved with a newer Keras version into an older one.
...
async def chat(request: ChatRequest):
    ...
    models_to_try = ["gemini-1.5-flash", "gemini-2.0-flash-exp", "gemini-1.5-pro", "gemini-flash-latest"]
    last_error = ""
